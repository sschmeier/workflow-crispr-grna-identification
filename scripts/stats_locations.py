...
def stats(infiles, info_file, bed_file, outfile):
    ...
    for f in infiles:
        ...
        num_grnas = df.shape[0]

        # number without ot
        num_zero_ot = df[df["offtarget_num"] == 0].shape[0]
        # what happens to NaNs?
        num_nans = df[df["offtarget_num"].isnull()].shape[0]
        ...
        if num_with_ot == 0 and num_zero_ot == 0:
            num_zero_ot = "-"
            num_with_ot = "-"
    ...
    out_header = "loc\ttx\tregion_searched\tnum_grna_selected\tnum_grna_without_offtargets\tnum_grna_with_offtargets\tavg_num_offtargets\tnum_nan(possibly offtargets not tested)\n"
